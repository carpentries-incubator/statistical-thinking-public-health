...
objectives:
  - "Explore the difference between a population parameter and sample estimates through simulation from a normal distribution."
  - "Define the 95% confidence interval through simulation from a normal distribution."
keypoints:
...
the US, to estimate the mean height. The standard error of this estimate 
would be `r round(6.89/sqrt(1000), 2)` cm. Therefore, on average, we expect
...
Let's take a look at how we would calculate the 95% confidence interval in R.
...
mean heights, the lower bound of the confidence intervals and the higher
...
> each. For each sample, store the sample ID, mean systolic blood pressure the 
